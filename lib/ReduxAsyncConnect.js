...
function loadAsyncConnect(_ref) {
  ...
  var fetchDeferred = _ref.fetchDeferred;

  var rest = _objectWithoutProperties(_ref, ['components', 'fetchDeferred']);

  var hasAsync = false;
  var loadDeferred = false;
  var promise = Promise.all(filterAndFlattenComponents(components).map(function (Component) {
    ...
    return Promise.all(asyncItems.reduce(function (itemsResults, item) {
      ...
      var loadItem = fetchDeferred ? item.deferred : item;
      ...
      return asyncItems.reduce(function (result, item, index) {
        return _extends({}, result, _defineProperty({}, item.key, results[index]));
      }, {});
    });
  }));
...
function loadOnServer(args) {
  ...
  if (result.hasAsync) {
    result.promise.then(function () {
      args.store.dispatch((0, _asyncConnect.endGlobalLoad)(result.loadDeferred));
    });
  }
...
var ReduxAsyncConnect = function (_React$Component) {
  ...
  _createClass(ReduxAsyncConnect, [{
    ...
    value: function componentDidMount() {
      ...
      var fetchDeferred = !this.isDeferredLoaded();

      if (!dataLoaded || fetchDeferred) {
    ...
    value: function componentWillReceiveProps(nextProps) {
      var deferredLoaded = this.isDeferredLoaded();
    ...
    key: 'isDeferredLoaded',
    value: function isDeferredLoaded() {
      return this.context.store.getState().reduxAsyncConnect.deferredLoaded;
    }
  }, {
    key: 'loadAsyncData',
    value: function loadAsyncData(props, fetchDeferred) {
      var _this2 = this;

      var store = this.context.store;
      var loadResult = loadAsyncConnect(_extends({}, props, { store: store, fetchDeferred: fetchDeferred }));
